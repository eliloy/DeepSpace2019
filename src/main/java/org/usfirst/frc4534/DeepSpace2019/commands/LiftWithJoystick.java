...
public class LiftWithJoystick extends Command {
    ...
    protected double liftStepSize = 0.02;
    ...
    protected void interrupted() {} 
        protected double scaleLiftSpeed(double Liftspeed, double lastLiftSpeed) {
        if (Math.abs(Liftspeed) <= 0.05 && Math.abs(lastLiftSpeed) <= 0.05) {
            return 0;
        } else if (Liftspeed > lastLiftSpeed) {
            if (Math.abs(Liftspeed - lastLiftSpeed) < liftStepSize) {
                return Liftspeed;
            } else {
                return lastLiftSpeed + liftStepSize;
            }
        } else if (Liftspeed < lastLiftSpeed) {
            if (Math.abs(Liftspeed - lastLiftSpeed) < liftStepSize) {
                return Liftspeed;
